...
// Mock child components to test ChatFooter in isolation
jest.mock("@/components/chat/footer/InputWithSend", () => ({
  InputWithSend: jest.fn(() => <div data-testid="input-with-send" />),
}))
jest.mock("@/components/chat/footer/ActionButtons", () => ({
  ActionButtons: jest.fn(() => <div data-testid="action-buttons" />),
}))
...
describe("ChatFooter Component", () => {
  ...
  test("renders the main child components", () => {
    render(<ChatFooter {...mockProps} />)
    expect(screen.getByTestId("input-with-send")).toBeInTheDocument()
    expect(screen.getByTestId("action-buttons")).toBeInTheDocument()
  })

  test("passes the correct props to InputWithSend", () => {
    const { InputWithSend } = require("@/components/chat/footer/InputWithSend")
    render(<ChatFooter {...mockProps} input="test" isLoading={true} />)
    expect(InputWithSend).toHaveBeenCalledWith(
      expect.objectContaining({
        input: "test",
        isLoading: true,
  ...
  test("passes the correct props to ActionButtons", () => {
    const { ActionButtons } = require("@/components/chat/footer/ActionButtons")
    render(<ChatFooter {...mockProps} />)
    expect(ActionButtons).toHaveBeenCalledWith(
      expect.objectContaining({
        onFileUpload: mockProps.onFileUpload,
        onImageUpload: mockProps.onImageUpload,
        openModal: mockProps.openModal,
      }),
      {},
    )
  })

  test("displays the keyboard shortcut hint text", () => {
    render(<ChatFooter {...mockProps} />)
