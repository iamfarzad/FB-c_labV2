...
interface DemoSessionContextType {
  ...
  featureUsage: Record<string, { tokensUsed: number; requestsMade: number; remainingTokens: number; remainingRequests: number }>
}
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.featureUsage) {
          const usage: Record<string, { tokensUsed: number; requestsMade: number; remainingTokens: number; remainingRequests: number }> = {}
...
export function DemoSessionStatus() {
  const { sessionId, sessionStatus, isLoading, refreshStatus, remainingTokens, remainingRequests, featureUsage } = useDemoSession()
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span>{remainingTokens.toLocaleString()} / {totalTokens.toLocaleString()}</span>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span>{remainingRequests} / {totalRequests}</span>
            </div>
        ...
        {sessionStatus?.isComplete && (
          <div className="p-3 bg-green-50 border border-green-200 rounded-md">
            <p className="text-sm text-green-800">
              🎉 Demo complete! You've explored all our AI capabilities.
            </p>
          </div>
        )}
      </CardContent>
