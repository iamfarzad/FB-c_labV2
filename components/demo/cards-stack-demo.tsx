    ...
    <div className="container min-h-svh place-content-center bg-stone-50 px-6 text-stone-900 xl:px-12">
      <div className="grid md:grid-cols-2 md:gap-8 xl:gap-12">
        <div className="left-0 top-0 md:sticky md:h-svh md:py-12">
          <h5 className="text-xs font-medium uppercase tracking-wider text-stone-500">
            our process
          </h5>
        ...
        <ContainerScroll className="min-h-[400vh] space-y-8 py-12">
          {PROCESS_PHASES.map((phase, index) => (
            <CardSticky
              key={phase.id}
              index={index + 2}
              className="rounded-2xl border bg-white p-8 shadow-md backdrop-blur-md"
            >
...
export const Work = () => {
  return (
    <div className="container min-h-svh place-content-center bg-slate-900 p-6 text-stone-50 md:p-12">
      <div className="text-center">
        <h5 className="text-xs font-medium uppercase tracking-wider text-stone-400">
          latest projects
        </h5>
        <h2 className="mb-4 mt-1 text-4xl font-bold tracking-tight">
          Get a glimpse of <span className="text-indigo-500">our work</span>
        </h2>
        <p className="mx-auto max-w-prose text-sm text-stone-400">
      ...
      <ContainerScroll className="min-h-[500vh] py-12">
        {WORK_PROJECTS.map((project, index) => (
          <CardSticky
            key={project.id}
            index={index}
            className="w-full overflow-hidden rounded-sm border-x border-indigo-900 border-t-indigo-500 border-b-indigo-900 bg-indigo-950"
            incrementY={60}
            incrementZ={5}
          >
            <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-4">
              ...
              <div className="flex flex-wrap gap-1">
                {project.services.map((service) => (
                  ...
                  >
                    <span className="text-xs tracking-tighter text-stone-300">
                      {service}
                    </span>
                  </div>
                ))}
...
export const Achievements = () => {
  return (
    <div className="min-h-svh bg-stone-100">
      <ContainerScroll className="min-h-[400vh] place-items-center space-y-8 p-6 text-center text-zinc-50 md:p-12">
