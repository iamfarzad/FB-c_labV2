...
const ProcessNode = ({ activity, index, isLatest, onHover, onLeave }: ProcessNodeProps) => {
  const getNodeStyle = (status: ActivityItem["status"]) => {
    const baseClasses = "relative flex items-center justify-center rounded-full border transition-all duration-300 ease-out"
  ...
  const getIcon = (type: ActivityItem["type"]) => {
    const iconMap = {
      ...
      email_sent: Mail,
      error: Activity, // Use neutral Activity icon instead of AlertTriangle
      web_scrape: Link,
      link: Link,
      vision_analysis: Eye,
      database: Database,
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      {showTooltip && hoveredActivity && (
        <div 
          className="absolute left-12 top-1/2 transform -translate-y-1/2 bg-popover/95 backdrop-blur-sm text-popover-foreground px-3 py-2 rounded-md shadow-xl border border-border pointer-events-none transition-all duration-200 whitespace-nowrap z-50"
        >
