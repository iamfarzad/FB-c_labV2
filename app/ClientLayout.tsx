...
// Removed useState, useEffect for manual theme handling
...
// import { WarpBackground } from "@/components/magicui/warp-background" // WarpBackground can be added back if needed
import "./globals.css"
...
export default function ClientLayout({
  ...
  // This gridColor logic might need to be adapted if WarpBackground is re-added,
  // potentially by reading theme from next-themes context or CSS variables.

  return (
    // ThemeProvider wraps the html tag or a high-level component.
    // Here, placing it just inside body to ensure html tag can get class.
    // Or, ThemeProvider could wrap the entire <html> element if structured differently.
    // For ShadCN, typical usage is to wrap content INSIDE body.
    // Let's ensure Tailwind's dark mode strategy (`darkMode: 'class' in tailwind.config.ts`) is used.
    <html lang="en" suppressHydrationWarning> {/* suppressHydrationWarning is often recommended with next-themes */}
      <body className={`${rajdhani.variable} ${spaceMono.variable} font-tech relative`}>
        ...
        >
          {/* Style tag with CSS variables can remain, as they are toggled by .dark/.light class */}
          <style jsx global>{`
            :root {
            ...
            /* The .dark and .light classes will be applied by next-themes to the HTML element */
            .dark {
          ...
          `}</style>
          {/* <WarpBackground gridColor={gridColor} /> Re-enable if needed, ensuring gridColor is theme-aware */}
          <div className="relative z-20"> {/* Ensure children are rendered above background if any */}
            {children}
          </div>
        </ThemeProvider>
      </body>
